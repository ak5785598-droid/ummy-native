    ...
    <Modal visible={visible} transparent animationType="slide" onRequestClose={onClose}>
      ...
      >
        <View className="flex-1 bg-black/60 justify-end">
          <View className="bg-white rounded-t-[2rem] h-[85%]">
            <View className="flex-row items-center justify-between px-6 pt-4 pb-3 border-b border-slate-100">
              <View className="flex-row items-center gap-2"><MessageCircle size={16} color="#64748b" /><Text className="text-slate-900 text-base font-bold">{selectedChat ? selectedChat.otherUser.name : 'Messages'}</Text></View>
              {selectedChat ? <TouchableOpacity onPress={() => setSelectedChat(null)}><Text className="text-purple-600 text-xs font-bold">Back</Text></TouchableOpacity> : <TouchableOpacity onPress={onClose} className="p-1"><X size={20} color="#64748b" /></TouchableOpacity>}
            </View>
            {!selectedChat ? (
