          ...
          <TouchableOpacity activeOpacity={1} style={styles.popupCard}>
            ...
            <ScrollView horizontal showsHorizontalScrollIndicator={false} contentContainerStyle={styles.scrollContent}>
              {levels.map((level: any, idx: number) => {
                ...
                return (
                  <View key={level.id} style={[styles.pathNode, isCur && { transform: [{ scale: 1.1 }] }]}>
                    <View
                      style={[
                        styles.nodeCircle,
                    ...
                    >
                      {isComp ? (
                        ...
                        LEVEL_IMAGES[level.id] ? (
                          <Image source={LEVEL_IMAGES[level.id]} style={styles.nodeImage} cachePolicy="memory-disk" />
                        ) : nodeImageUrl ? (
                          isNodeSvg ? (
