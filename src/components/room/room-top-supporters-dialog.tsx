...
function isThisWeek(millis: number): boolean {
  const d1 = new Date(millis);
  const d2 = new Date();
...
const LiveSupporterCard = React.memo(function LiveSupporterCard({
  supporter,
  render
}: {
  supporter: any;
  render: (live: { username: string; avatarUrl: string }) => React.ReactNode;
}) {
  const { profile } = useUserProfile(supporter?.uid);
  const username = profile?.username || profile?.name || supporter?.username || 'User';
  const avatarUrl = profile?.avatarUrl || supporter?.avatarUrl || '';
  return <>{render({ username, avatarUrl })}</>;
});
...
function SupporterProfilePopup({
  ...
  return (
    <LiveSupporterCard supporter={supporter} render={({ username, avatarUrl }) => (
          ...
          <View style={{ backgroundColor: '#0f172a', borderRadius: 24, width: '100%', overflow: 'hidden', borderWidth: 1, borderColor: 'rgba(255,255,255,0.1)' }}>
            <LinearGradient colors={['#1e293b', '#0f172a']} style={{ alignItems: 'center', paddingTop: 32, paddingBottom: 24, paddingHorizontal: 24 }}>
              ...
              <View style={{ borderColor: cfg.border, borderWidth: 3, borderRadius: 50, marginBottom: 12 }}>
                <Image cachePolicy="memory-disk" source={{ uri: toCDN(avatarUrl) || 'https://picsum.photos/100' }} style={{ width: 80, height: 80, borderRadius: 40 }} />
              </View>
              <Text style={{ color: 'white', fontSize: 18, fontWeight: '900', marginBottom: 2 }}>{username}</Text>
  ...
  const getSortedSupporters = () => {
    return supporters
      .map(s => {
        ...
        let displayAmount = 0;
        if (activeTab === 'daily') {
          // If updated today, use dailyAmount or amount; else 0
          displayAmount = isToday(millis) ? (s.dailyAmount || s.amount || 0) : 0;
        } else if (activeTab === 'weekly') {
          // If updated this week, use weeklyAmount or amount; else 0
          displayAmount = isThisWeek(millis) ? (s.weeklyAmount || s.amount || 0) : 0;
        } else {
          // Total / All Time
          displayAmount = s.amount || (s as any).totalAmount || s.dailyAmount || 0;
        }
            ...
            <ScrollView showsVerticalScrollIndicator={false}>
              ...
              {sorted.length > 0 ? (
                <View style={{ flexDirection: 'row', alignItems: 'flex-end', justifyContent: 'center', paddingHorizontal: 16, paddingTop: 24, paddingBottom: 32, gap: 8 }}>
                  ...
                  >
                    {top3[1] ? (
                      <LiveSupporterCard supporter={top3[1]} render={({ username, avatarUrl }) => (
                        <>
                          ...
                          <View style={{ position: 'relative' }}>
                            <View style={{ borderWidth: 2.5, borderColor: borderColors[1], borderRadius: 40, padding: 2 }}>
                              <Image cachePolicy="memory-disk" source={{ uri: toCDN(avatarUrl) || 'https://picsum.photos/100' }} style={{ width: avatarSizes[1], height: avatarSizes[1], borderRadius: 30 }} />
                            </View>
                          ...
                          <Text style={{ color: '#cbd5e1', fontSize: 11, fontWeight: '900', marginTop: 10, textAlign: 'center' }} numberOfLines={1}>{username}</Text>
                  ...
                  >
                    {top3[0] ? (
                      <LiveSupporterCard supporter={top3[0]} render={({ username, avatarUrl }) => (
                        <>
                          ...
                          <View style={{ position: 'relative' }}>
                            ...
                            <View style={{ borderWidth: 3, borderColor: borderColors[0], borderRadius: 40, padding: 2 }}>
                              <Image cachePolicy="memory-disk" source={{ uri: toCDN(avatarUrl) || 'https://picsum.photos/100' }} style={{ width: avatarSizes[0], height: avatarSizes[0], borderRadius: 34 }} />
                            </View>
                          ...
                          <Text style={{ color: '#fbbf24', fontSize: 12, fontWeight: '900', marginTop: 10, textAlign: 'center' }} numberOfLines={1}>{username}</Text>
                  ...
                  >
                    {top3[2] ? (
                      <LiveSupporterCard supporter={top3[2]} render={({ username, avatarUrl }) => (
                        <>
                          ...
                          <View style={{ position: 'relative' }}>
                            <View style={{ borderWidth: 2.5, borderColor: borderColors[2], borderRadius: 35, padding: 2 }}>
                              <Image cachePolicy="memory-disk" source={{ uri: toCDN(avatarUrl) || 'https://picsum.photos/100' }} style={{ width: avatarSizes[2], height: avatarSizes[2], borderRadius: 28 }} />
                            </View>
                          ...
                          <Text style={{ color: '#d97706', fontSize: 11, fontWeight: '900', marginTop: 10, textAlign: 'center' }} numberOfLines={1}>{username}</Text>
              ...
              {sorted.length === 0 ? (
                ...
                <View style={{ paddingBottom: 40, paddingHorizontal: 16 }}>
                  {rest.map((s, idx) => (
                    <LiveSupporterCard key={s.uid || idx} supporter={s} render={({ username, avatarUrl }) => (
                      <TouchableOpacity
                      ...
                      >
                        ...
                        <Image cachePolicy="memory-disk" source={{ uri: toCDN(avatarUrl) || 'https://picsum.photos/100' }} style={{ width: 40, height: 40, borderRadius: 20, marginRight: 12, borderWidth: 1, borderColor: 'rgba(255,255,255,0.1)' }} />
                        <View style={{ flex: 1 }}>
                          <Text style={{ color: 'white', fontSize: 13, fontWeight: '900' }} numberOfLines={1}>{username}</Text>
                        </View>
