...
const ChatMessageRow = React.memo(function ChatMessageRow({ message, onPress, onAvatarPress, onImagePress, onTranslate, translationText, isTranslating, canManage, onDeleteMessage }: ChatMessageRowProps) {
  ...
  // SYSTEM_BOT = Ummy AI — special purple AI bubble
  if (message.senderId === 'SYSTEM_BOT') {
    return (
      <View style={styles.aiBotRow}>
        <Image cachePolicy="memory-disk" source={{ uri: 'https://img.icons8.com/isometric/512/bot.png' }} style={styles.aiBotAvatar} />
        <View style={styles.aiBotContentWrapper}>
          <View style={styles.aiBotNameRow}>
            <Text style={styles.aiBotName}>✨ Ummy AI</Text>
          </View>
          <View style={styles.aiBotBubble}>
            <Text style={styles.aiBotText}>{message.content || message.text}</Text>
          </View>
        </View>
      </View>
    );
