...
export function TeenPattiGame({ onClose, roomId, onRoundEnd, isMuted }: TeenPattiGameProps) {
  ...
  const saveBetsToRTDB = (bets: Record<string, number>) => {
    ...
    const gamePath = `games/teen_patti_${roomId}/playerBets/${currentUser.uid}`;
  ...
  useEffect(() => {
    ...
    const playerBetsPath = `games/teen_patti_${roomId}/playerBets/${currentUser.uid}`;
    ...
    databaseGet(databaseRef(database, playerBetsPath)).then((snap: any) => {
      ...
      databaseGet(databaseRef(database, gamePath)).then((gameSnap: any) => {
        ...
        const winId = gameData?.winId;
        if (winId) {
          const betAmt = data.bets[winId] || 0;
  ...
  const handlePlaceBet = async (factionId: string) => {
    ...
    setMyBets(prev => {
      const next = { ...prev, [factionId]: (prev[factionId] || 0) + selectedChip };
  ...
  const finalizeRound = async (winId: string) => {
    ...
    if (database && roomId && currentUser) {
      try {
        const playerBetsRef = databaseRef(database, `games/teen_patti_${roomId}/playerBets`);
    ...
    if (onRoundEnd) {
      onRoundEnd({
        ...
        myPrize: winAmount,
        myWager: myBetsRef.current[winId] || 0
      });
    }
  };
