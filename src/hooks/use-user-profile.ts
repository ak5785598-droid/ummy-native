  ...
  useEffect(() => {
    ...
    const mergeAndSet = () => {
      try {
        ...
        const subCoins = subWallet.coins ?? (sub as any)?.coins;
        const baseCoins = baseWallet.coins ?? (base as any)?.coins;
        const finalCoins = subCoins !== undefined ? subCoins : (baseCoins !== undefined ? baseCoins : 0);
