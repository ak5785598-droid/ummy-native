...
export function useVoiceCaptions(roomId: string, isInSeat: boolean, isMuted: boolean) {
  ...
  useEffect(() => {
    ...
    const unsub = onSnapshot(q, (snap) => {
      ...
      snap.forEach(d => { list.push(d.data() as CaptionData); });
  ...
  // Native STT via react-native-voice
  ...
  useEffect(() => {
    ...
    if (Voice) {
      setSttEngine('native');
      setupNativeSTT();
    } else if (Platform.OS === 'web' && ((window as any).SpeechRecognition || (window as any).webkitSpeechRecognition)) {
      setSttEngine('web');
      setupWebSTT();
    } else {
      setSttEngine('unavailable');
    }
