  ...
  const handleSpin = async () => {
    ...
    Animated.timing(spinAnim, { toValue: targetAngle, duration: 3000, easing: Easing.out(Easing.cubic), useNativeDriver: true }).start(async () => {
      ...
      try {
        ...
        batch.update(doc(firestore, 'users', user.uid), { 'wallet.coins': increment(winAmount - cost), updatedAt: serverTimestamp() });
  ...
  const handleRoll = async () => {
    ...
    animateDice(cpuAnim, 0, () => {
      ...
      if (w === 'win') {
        ...
        try {
          const batch = writeBatch(firestore);
          batch.update(doc(firestore, 'users', user.uid), { 'wallet.coins': increment(winAmount - bet), 'stats.dailyGameWins': increment(1), updatedAt: serverTimestamp() });
        ...
        try {
          const batch = writeBatch(firestore);
          batch.update(doc(firestore, 'users', user.uid), { 'wallet.coins': increment(-bet), updatedAt: serverTimestamp() });
...
function GoldenChest({ visible, onClose }: { visible: boolean; onClose: () => void }) {
  ...
  const handleOpen = async () => {
    ...
    setTimeout(async () => {
      try {
        const batch = writeBatch(firestore);
        batch.update(doc(firestore, 'users', user.uid), { 'wallet.coins': increment(coinsReward), 'wallet.diamonds': increment(diamondReward), lastChestOpen: serverTimestamp(), updatedAt: serverTimestamp() });
