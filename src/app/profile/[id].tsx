...
export default function ProfileScreen() {
  ...
  useEffect(() => {
    ...
    getDoc(ref).then((snap: any) => {
      ...
      setIsFollowing(snap?.exists || false);
    });
    ...
    <SafeAreaView style={{ flex: 1, backgroundColor: 'white' }} edges={['top']}>
      ...
      >
        <View style={{ maxWidth: 440, alignSelf: 'center', paddingHorizontal: 20 }}>
          {/* Avatar + Info Row */}
          <View style={{ flexDirection: 'row', alignItems: 'flex-start', gap: 4, marginBottom: 8 }}>
            {/* Avatar with Frame */}
            ...
            {/* Name + Tags */}
            <View style={{ flex: 1, minWidth: 0, paddingLeft: 4, paddingTop: 4 }}>
                ...
                <TouchableOpacity onPress={handleCopyId}>
                  {profile.tags?.includes('Official') ? (
                    ...
                    <View style={{ backgroundColor: '#f1f5f9', borderRadius: 6, paddingHorizontal: 8, paddingVertical: 2 }}>
                      <Text style={{ fontSize: 12, fontWeight: '700', color: '#475569' }}>ID: {displayID}</Text>
                    </View>
                  )}
                </TouchableOpacity>
