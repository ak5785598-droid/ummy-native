...
import { collection, query, where, orderBy, limit, doc, serverTimestamp, deleteDoc, updateDoc, arrayUnion, arrayRemove, runTransaction, onSnapshot } from '@/firebase/firestore-compat';
...
export default function MessagesScreen() {
  ...
  // Dynamic hiding of bottom tab bar on expo-router
  useEffect(() => {
...
function ChatListItem({ chat, currentUid, onPress, onLongPress }: { chat: PrivateChat; currentUid: string; onPress: () => void; onLongPress?: () => void }) {
  const router = useRouter();
...
function ChatRoomScreen({ chatId, recipientUid, onBack }: { chatId: string; recipientUid: string; onBack: () => void }) {
  const { user } = useUser();
  const firestore = useFirestore();
  const storage = useStorage();
  const navigation = useNavigation();
  ...
  // Guarantee bottom nav is hidden inside active chat
  useEffect(() => {
    navigation.getParent()?.setOptions({
      tabBarStyle: { display: 'none' }
    });
