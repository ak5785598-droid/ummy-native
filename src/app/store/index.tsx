...
import { View, Text, ScrollView, TouchableOpacity, Alert, Modal, ActivityIndicator, StyleSheet, FlatList, Dimensions } from 'react-native';
import { SafeAreaView } from 'react-native-safe-area-context';
import {
  ChevronLeft, ShoppingBag, Check, X, Play,
  Activity, Palette, MessageSquare, Ticket, ImageIcon
} from 'lucide-react-native';
import { useRouter } from 'expo-router';
import { useUser, useFirestore } from '@/firebase/provider';
import { useUserProfile } from '@/hooks/use-user-profile';
import { collection, query, orderBy, onSnapshot, doc, increment, serverTimestamp, arrayUnion, updateDoc } from '@/firebase/firestore-compat';
...
const STATIC_ENTRY_ITEMS = [
  { id: 'e-line', name: 'Line Entry', type: 'Entry', entryType: 'line', price: 5000, durationDays: 30, description: 'Grand full-screen video entrance effect for your room entry.', color: '#fbbf24' },
  { id: 'e-lion', name: 'Lion Roar', type: 'Entry', entryType: 'lion', price: 10000, durationDays: 30, description: 'Fiery golden lion Lottie entrance — roar into the room.', color: '#f97316' },
];

const STATIC_FRAME_ITEMS = [
  { id: 'f-fire-ring', name: 'Fire Ring Frame', type: 'Frame', price: 500000, durationDays: 30, description: 'Animated fire ring avatar frame with blazing flames.', videoUrl: 'https://firebasestorage.googleapis.com/v0/b/studio-7826224327-e0efc.firebasestorage.app/o/store%2Fframes%2Ffire_ring_final.mp4?alt=media&token=3ec511c2-9fc4-49be-a0a7-a781f25f1dae' },
];
...
export default function StoreScreen() {
  ...
  const handlePurchase = async () => {
    ...
    try {
      ...
      await updateDoc(userRef, { 'wallet.coins': increment(-finalPrice), updatedAt: serverTimestamp() });
  ...
  const renderItem = ({ item }: { item: any }) => {
    ...
    const isVideo = item.videoUrl || (item.imageUrl && (item.imageUrl.includes('.mp4') || item.imageUrl.includes('video')));
            ...
            {previewItem && (
              <>
                ...
                <View style={styles.modalMedia}>
                  {(previewItem.videoUrl || (previewItem.imageUrl && previewItem.imageUrl.includes('.mp4'))) ? (
                    <Video
                      source={{ uri: previewItem.videoUrl || previewItem.imageUrl }}
                      style={styles.modalMediaFill}
                      resizeMode={ResizeMode.CONTAIN}
                      shouldPlay isLooping isMuted
                    />
                  ) : previewItem.imageUrl ? (
                    <Image cachePolicy="memory-disk" source={{ uri: previewItem.imageUrl }} style={styles.modalMediaFill} contentFit="contain" />
                  ) : (
                ...
                {isItemOwned(previewItem.id) ? (
                  ...
                  <TouchableOpacity
                    style={[styles.buyBtn, isProcessing && { opacity: 0.6 }]}
                  ...
                  >
                    {isProcessing ? (
                      ...
                      <Text style={styles.buyBtnText}>Buy for 🪙 {getPrice(previewItem.price, selectedDuration).toLocaleString()}</Text>
                    )}
                  </TouchableOpacity>
                )}
              </>
...
const styles = StyleSheet.create({
  ...
  itemMedia: { width: '100%', height: 120, backgroundColor: '#f8fafc', position: 'relative', alignItems: 'center', justifyContent: 'center' },
  ...
  modalMedia: { width: '100%', height: 180, backgroundColor: '#f8fafc', borderRadius: 16, overflow: 'hidden', marginBottom: 16 },
